...
public class SnowIsland implements Runnable{
    int reds = 12, greens = 13, blues = 14;

    @Override
    public void run() {
        List<String> games = List.of(input.split("\n"));
        System.out.println("running");
        int idTotals = getValidGameIdTotals(games);
        System.out.printf("Total of Ids for Games which are valid with %d red, %d blue, and %d green is '%d'", reds, blues, greens, idTotals);
        int power = 0;
        power = getPower(games);
        System.out.println("the total power of all games is " + power);
        System.out.println("end");
    }

    private int getPower(List<String> games) {
        return games.stream()
                .filter(g -> !g.isEmpty())
                .map(this::buildResult)
//                .peek(System.out::println)
                .map(g -> g.getMaxBlue() * g.getMaxGreen() * g.getMaxRed())
//                .peek(System.out::println)
                .reduce(Integer::sum)
                .orElse(0);
    }

    private Integer getValidGameIdTotals(List<String> games) {
        return games.stream()
                .filter(g -> !g.isEmpty())
                .map(this::buildResult)
//                .peek(System.out::println)
                .filter(result -> result.getMaxRed() <= reds)
                .filter(result -> result.getMaxGreen() <= greens)
                .filter(result -> result.getMaxBlue() <= blues)
//                .peek(System.out::println)
                .map(Result::getGameNum)
                .reduce(Integer::sum)
                .orElse(0);
    }

    private Result buildResult(String game) {
        Result result = new Result();
        String[] line = game.split(":");

        extractGameNum(line[0], result);
        extractMaxColors(line[1], result);


        return result;
    }

    private void extractMaxColors(String sets, Result result) {
        String[] pulls = sets.split(";");
        Arrays.stream(pulls)
                .flatMap(pull -> Arrays.stream(pull.split(","))) // emit each element of the substream back onto main stream
                .forEach(color -> setMaxColor(result, color.trim()));

        //probably much easier to read than the stream.
//        for (String pull : pulls) {
//            for (String color : pull.split(",")) {
//                setMaxColor(result, color);
//            }
//        }
    }

    private void setMaxColor(Result result, String color) {
        if(color.endsWith("red")){
            result.setMaxRed(Integer.parseInt(color.split(" ")[0]));
        } else if(color.endsWith("blue")){
            result.setMaxBlue(Integer.parseInt(color.split(" ")[0]));
        } else if(color.endsWith("green")){
            result.setMaxGreen(Integer.parseInt(color.split(" ")[0]));
        }
    }

    private static void extractGameNum(String line, Result result) {
        String gamenum = line.replaceAll("[\\D]", "");
        result.setGameNum(gamenum.isEmpty() ? -1 : Integer.parseInt(gamenum));
    }

    private final String input = "Game 1: 4 blue; 1 green, 2 red; 4 blue, 1 green, 6 red\n" +
